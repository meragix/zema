...
class ZemaCoerce {
  ...
  static ZemaSchema<dynamic, int> integer({int? min, int? max}) =>
      CoerceInt(min: min, max: max);

  /// Coerce to boolean from string or number
  static ZemaSchema<dynamic, bool> boolean() => const CoerceBool();

  /// Coerce to double from string or number
  static ZemaSchema<dynamic, double> number({double? min, double? max}) =>
      CoerceDouble(min: min, max: max);

  /// Coerce to string from any value
  static ZemaSchema<dynamic, String> string() => const CoerceString();
}
