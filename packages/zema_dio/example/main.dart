...
Future<void> main() async {
  ...
  await _exampleSafeParse(dio);
  await _exampleParse(dio);
  await _exampleList(dio);
  await _exampleSchemaFailure(dio);
...
Future<void> _exampleSafeParse(Dio dio) async {
  print('\n--- safeParse ---');

  final response = await dio.get<dynamic>('/users/1');
...
Future<void> _exampleParse(Dio dio) async {
  print('\n--- parse ---');

  try {
    final response = await dio.get<dynamic>('/users/2');
...
Future<void> _exampleList(Dio dio) async {
  print('\n--- list (array schema) ---');

  final response = await dio.get<dynamic>('/posts');
...
Future<void> _exampleSchemaFailure(Dio dio) async {
  print('\n--- schema mismatch ---');
